c ***********************************************************************
c
c $HeadURL: svn://129.13.162.100/simson/blaq/rparambl.f $
c $LastChangedDate: 2016-02-02 18:56:51 +0100 (Tue, 02. Feb 2016) $
c $LastChangedBy: alex $
c $LastChangedRevision: 87 $
c
c ***********************************************************************
     ...
     &     adjoint,rgh_h,rgh_ampl,rgh_maxyn,rgh_hy,
     &     nzstat,nystat,ystat,zstat,namzstat,namystat,
     &     fd,fd1)
c
...
c
c     plane statistics ***Pourya***
c
      integer nzstat,nystat,zstat(nzstat0),ystat(nystat0)
      character*80 namzstat,namystat
c
...
C     Roughness with direct forcing (loctyp==9,10)
      ...
      real rgh_hy(nxp,nyp)
      real rgh_ampl
      integer rgh_maxyn,rgh_xis,rgh_yis,rgh_zis
...
c
      ...
      if (loctyp.eq.9) then
        open(unit=711,status='old',file='rgh_h.bin',form='binary')
        if (my_node.eq.0) then
         write(*,*) 'Reading distribution file...'
        end if
        read(711) rgh_xis
        read(711) rgh_zis
        read(711) rgh_maxyn
        read(711) rgh_ampl
         if(rgh_xis.ne.nxp)then
          if (my_node.eq.0) then
           write(*,*) 'x-dimension does not match!'
          end if
          call stopnow(134223265)
         endif
         if(rgh_zis.ne.nzp)then
          if (my_node.eq.0) then
           write(*,*) 'z-dimension does not match!'
          end if
          call stopnow(343338465)
         endif
        read(711) rgh_h
        close(711)
        if (my_node.eq.0) then
        write(*,*) 'Roughness height dimensions: ',rgh_xis,' x ',rgh_zis
            write(*,*) '   rgh_maxyn   ',rgh_maxyn
            write(*,*) '   rgh_ampl    ',rgh_ampl
        end if
      end if
      if (loctyp.eq.10) then
        open(unit=711,status='old',file='ibm.bin',form='binary')
        if (my_node.eq.0) then
         write(*,*) 'Reading distribution file...'
        end if
        read(711) rgh_xis
        ...
        read(711) rgh_ampl
         if(rgh_xis.ne.nxp)then
          if (my_node.eq.0) then
           write(*,*) 'x-dimension does not match!'
           write(*,*) 'x=',rgh_xis,', needed ',nxp
          end if
          call stopnow(134223265)
         endif
         if(rgh_yis.ne.nyp)then
          if (my_node.eq.0) then
           write(*,*) 'y-dimension does not match!'
           write(*,*) 'y=',rgh_yis,', needed ',nyp
          end if
          call stopnow(323423465)
         endif
        read(711) rgh_hy
        close(711)
        if (my_node.eq.0) then
        write(*,*) 'Damping distribution dimensions: '
     &             ,rgh_xis,' x ',rgh_yis
            write(*,*) '   rgh_maxyn   ',rgh_maxyn
            write(*,*) '   rgh_ampl    ',rgh_ampl
        end if
      ...
      if (loctyp.eq.11) then
        open(unit=711,status='old',file='fd.bin',form='binary')
        if (my_node.eq.0) then
         write(*,*) 'Reading body force dist file...'
        end if
        read(711) fd_yis
        if(fd_yis.ne.nyp)then
          if (my_node.eq.0) then
           write(*,*) 'y-dimension does not match!'
          end if
        call stopnow(134223265)
        ...
        close(711)
        if (my_node.eq.0) then
        write(*,*) 'Body force dimension: ',rgh_yis
        end if
      end if   
c
...
c
c     number of xy planes for statistics (nzstat) ***Pourya***
c 
      call comment(10)
      read(10,*) nzstat
      nzstat=max(0,nzstat)
      if (my_node.eq.0) then
         write(*,*) 'Number of xy planes (nzstat) : ', nzstat
      end if
      if (nzstat.gt.0) then
         do i=1,nzstat
            call comment(10)
            read(10,*) zstat(i)
            if (my_node.eq.0) then
               write(*,*) 'xy planes index (zstat) : ', zstat(i)
            end if
         end do
         call comment(10)
         read(10,'(a)') namzstat
...
c
c     number of xz planes for statistics (nystat) ***Pourya***
c
      call comment(10)
      read(10,*) nystat
      nystat=max(0,nystat)
      if (my_node.eq.0) then
         write(*,*) 'Number of xz planes (nystat): ', nystat
      end if
      if (nystat.gt.0) then
         do i=1,nystat
            call comment(10)
